...
import { DEFAULT_STYLE_IMAGE_PROMPT } from "@/lib/pipeline/commonPromptDefaults";
...
const PROMPT_FILENAME = "style-image-prompt.txt";

/** Generates (or regenerates, replacing the previous one) the project's 톤앤매너 기준 이미지 from a prompt — follows the project's image-engine.txt choice (OpenAI/cloud vs local FLUX.2 Klein), same as scene image generation. */
export async function POST(req: NextRequest, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  const prompt = (typeof body?.prompt === "string" ? body.prompt : "").trim() || DEFAULT_STYLE_IMAGE_PROMPT;
  ...
  try {
    return await withInFlightLock(`images:${projectId}:style-reference`, async () => {
      let buffer: Buffer;
      if (engine === "local") {
        let localClient;
        try {
          ...
          console.error("톤앤매너 기준 이미지 생성 실패:", err);
        ...
        try {
          ...
          await localClient.generateBatch([{ sceneId: `reference-style-${projectId}`, prompt }], {
      ...
      await writeProjectReferenceImage(projectId, "style", buffer);
      await writeProjectFile(projectId, PROMPT_FILENAME, prompt);
      return NextResponse.json({ ok: true });
    });
  } catch (err) {
    if (err instanceof AlreadyInFlightError) {
      return NextResponse.json({ error: "톤앤매너 기준 이미지가 이미 생성 중입니다. 완료될 때까지 기다려주세요." }, { status: 409 });
    }
    throw err;
  }
}
