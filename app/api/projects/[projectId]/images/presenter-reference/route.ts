...
const KIND = "presenter" as const;

/** Serves the project's presenter/announcer reference image, if one was generated or uploaded. */
export async function GET(_req: NextRequest, { params }: { params: Promise<{ projectId: string }> }) {
  const { projectId } = await params;
  const buffer = await readProjectReferenceImage(projectId, KIND);
  if (!buffer) return NextResponse.json({ error: "강사 참고 이미지를 찾을 수 없습니다" }, { status: 404 });
...
/** Saves a directly uploaded presenter reference image, replacing any existing one. */
...
/** Removes the saved presenter reference image. */
