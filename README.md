...
- `OPENAI_API_KEY` — 5단계 이미지 생성(선택 사항, 실제 과금 발생)에 사용. 이미지 생성을 안 쓸 거면 생략해도 나머지 단계는 정상 동작한다.
...
| 5. 이미지/목업 생성 (선택) | `images/{sceneId}.png` | OpenAI 이미지 API로 씬별 이미지 생성(제목 씬 제외, 실제 과금). 동시 생성 그룹은 화면 설계와 동일한 기준으로 묶이며, 실패 시 재시도(일반 오류 5초 후 1회, rate limit 30초 후 2회) 후에도 실패하면 사유를 표시하고 중단 |
...
`python/tts/.venv`에 가상환경을 만들고 `mlx-audio`를 설치한다. 모델 가중치(수백 MB~수 GB)는 최초 음성 생성 실행 시 Hugging Face 캐시(`~/.cache/huggingface`)로 자동 다운로드되므로 첫 요청은 시간이 좀 걸린다.
