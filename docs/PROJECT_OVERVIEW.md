...
(2026-08-15: 시퀀스 이미지 생성 방식 사이드카 부재 시 UI/백엔드 기본값이 서로 달랐던 문제는 백엔드 세 라우트를 모두 `"ai"` 기본값으로 통일해 해결함 — 5절 참고.)
